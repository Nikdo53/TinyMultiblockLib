...
public class VoxelShapeUtils {
    ...
    public static VoxelShape rotateAll(Direction to, VoxelShape shape){
        if (to == Direction.NORTH)
            return shape;
        if (isY(to)){
            return rotateShapeAroundX(Direction.NORTH, to, shape);
        } else
            return rotateShapeAroundY(Direction.NORTH, to, shape);
    }
    ...
    private static VoxelShape orUnoptimized(VoxelShape first, VoxelShape second)
    {
        return Shapes.joinUnoptimized(first, second, BooleanOp.OR);
    }

    private static boolean isX(Direction dir) {
    ...
    private static boolean isZ(Direction dir) {
